...
class InternacionController extends Controller
{
   public function create(Request $req, Response $res, $args = [])
   {
		...
		$cama_id = filter_var($data['cama_id'], FILTER_SANITIZE_NUMBER_INT);
		$fecha = filter_var($data['fecha']);
		$hora = filter_var($data['hora']);
		$sql = 'SELECT insertar_internacion(?,?,?,?)';
		try {
			$stmt = $this->db->prepare($sql);
			$stmt->bindParam(1, $diag, \PDO::PARAM_INT);
			$stmt->bindParam(2, $cama_id, \PDO::PARAM_INT);
			$stmt->bindParam(3, $fecha);
			$stmt->bindParam(4, $hora);
			$stmt->execute();
			$out = array('status' => '1', 'internacion' => $stmt->fetchColumn());
   ...
   public function read(Request $req, Response $res, $args = [])
   {
		...
		$sql = 'SELECT * FROM obtener_internacion(?)';
		try {
			$stmt = $this->db->prepare($sql);
			$stmt->bindParam(1, $id, \PDO::PARAM_INT);
			$stmt->execute();
			$out = array('status' => '1', 'internacion' => $stmt->fetch());
			return $res->withJson($out);
		} catch (\PDOException $e) {
			$out = array('status' => '0', 'error' => $stmt->errorInfo());
			return $res->withJson($out); 
   ...
   public function update(Request $req, Response $res, $args = [])
   {
		...
		$fecha = filter_var($data['fecha']);
		$hora = filter_var($data['hora']);
		$sql = 'SELECT actualizar_internacion(?,?,?,?,?)';
		try {
			$stmt = $this->db->prepare($sql);
			$stmt->bindParam(1, $id, \PDO::PARAM_INT);
			$stmt->bindParam(2, $diag, \PDO::PARAM_INT);
			$stmt->bindParam(3, $cama_id, \PDO::PARAM_INT);
   ...
   public function destroy(Request $req, Response $res, $args = [])
   {
		$out = null;
		$id = filter_var($args['id'], FILTER_SANITIZE_NUMBER_INT);
		$sql = 'SELECT eliminar_internacion(?)';
		try {
			$stmt = $this->db->prepare($sql);
			$stmt->bindParam(1, $id, \PDO::PARAM_INT);
			$stmt->execute();
   ...
   {
		$out = null;
		$sql = 'SELECT * FROM obtener_internaciones()';
		try {
			$stmt = $this->db->prepare($sql);
			$stmt->execute();
			$stmt->setFetchMode(\PDO::FETCH_ASSOC);
			$out = array('status' => '1', 'internaciones' => $stmt->fetchAll());
			return $res->withJson($out);
		} catch (\PDOException $e) {
			$out = array('status' => '0', 'error' => $stmt->errorInfo());
			return $res->withJson($out); 
